...
def train_one_epoch(
    ...
    for batch_id, sample_batched in enumerate(dataloader):
        ...
        loss = sum(
            [criterion(preds, labels, l_w) for preds, l_w in zip(preds_list, l_weight)]
        )  # bdcn_loss
        ...
        if batch_id % log_interval_vis == 0:
            ...
            vis_imgs = cv2.resize(
                vis_imgs, (int(vis_imgs.shape[1] * 0.8), int(vis_imgs.shape[0] * 0.8))
            )
...
def validate_one_epoch(epoch, dataloader, model, device, output_dir, arg=None):
    ...
    with torch.no_grad():
        for _, sample_batched in enumerate(dataloader):
            ...
            save_image_batch_to_disk(
                preds[-1], output_dir, file_names, img_shape=image_shape, arg=arg
            )


def test(checkpoint_path, dataloader, model, device, output_dir, args=None):
    ...
    with torch.no_grad():
        total_duration = []
        for batch_id, sample_batched in enumerate(tqdm(dataloader)):
            images = sample_batched["images"].to(device)
            ...
            save_image_batch_to_disk(
                preds, output_dir, file_names, image_shape, arg=args
            )
            torch.cuda.empty_cache()

    total_duration = np.sum(np.array(total_duration))
    print("******** Testing finished in dataset. *****")
...
def parse_args():
    ...
    parser.add_argument(
        "--is_testing", type=bool, default=is_testing, help="Script in testing mode."
    )
    parser.add_argument(
        "--double_img",
        type=bool,
    ...
    parser.add_argument(
        "--resume", type=bool, default=False, help="use previous trained data"
    )  # Just for test
    parser.add_argument(
        "--checkpoint_data",
        type=str,
    ...
    parser.add_argument(
        "--res_dir", type=str, default="result", help="Result directory"
    )
    parser.add_argument(
        "--log_interval_vis",
        type=int,
    ...
    parser.add_argument(
        ...
        default=1e-8,
        metavar="WD",
        help="weight decay (Good 1e-8) in TF1=0",
    )  # 1e-8 -> BIRND/MDBD, 0.0 -> BIPED
    parser.add_argument(
        "--adjust_lr", default=[10, 15], type=int, help="Learning rate step size."
    )  # [5,10]BIRND [10,15]BIPED/BRIND
    parser.add_argument(
        "--batch_size",
        type=int,
        default=8,
    ...
    parser.add_argument(
        "--tensorboard", type=bool, default=True, help="Use Tensorboard for logging."
    ),
    parser.add_argument(
        "--img_width", type=int, default=352, help="Image width for training."
    )  # BIPED 400 BSDS 352/320 MDBD 480
    parser.add_argument(
...
def main(args):
    ...
    checkpoint_path = os.path.join(
        args.output_dir, args.train_data, args.checkpoint_data
    )
    ...
    if not args.is_testing:
        ...
        dataset_train = BipedDataset(
            ...
            if len(args.mean_pixel_values) == 4
    ...
    dataloader_val = DataLoader(
        dataset_val, batch_size=1, shuffle=False, num_workers=args.workers
    )
    # Testing
    if args.is_testing:
